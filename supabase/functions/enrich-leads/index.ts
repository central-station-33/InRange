...
serve(async (req) => {
  ...
  for (const lead of leads) {
    ...
    try {
      ...
      const motivationScore = clampInt(result.motivation_score, 1, 5);

      // FIXED 2026-09-21 (was: trusting the model's own `routing` string
      // outright, see deriveRouting()'s comment for why). Derived here, not
      // read from the model.
      const routing = deriveRouting(bantScore, motivationScore);
...
/**
 * Derives `routing` from the scores instead of trusting the model's own
 * `routing` string (FIXED 2026-09-21). The prompt already states these exact
 * rules and asks the model to apply them, but checked against all enriched
 * leads to date, ~12% of the model's routings contradicted its own stated
 * rules -- including identical duplicate leads (same bant_score, same
 * motivation_score) that came back with different routings on different
 * runs. bantScore and motivationScore are already computed from the model's
 * own component scores by the time this runs, so deriving routing from them
 * here is free, deterministic, and can't drift from the policy the prompt
 * describes. A null score (missing/unparseable model output) is treated as
 * 0 -- the safe floor, not an inflated guess.
 */
function deriveRouting(bantScore: number | null, motivationScore: number | null): string {
  const bant = bantScore ?? 0;
  const motivation = motivationScore ?? 0;
  if (bant >= 9 && motivation >= 4) return 'hot';
  if (bant >= 7 || motivation >= 3) return 'warm';
  if (bant >= 4) return 'nurture';
  return 'cold';
}
