...
serve(async (req) => {
  ...
  for (const raw of leads) {
    ...
    try {
      // Case-insensitive, comma-safe duplicate lookup, matching the natural
      // key isa_leads_natural_key_uidx enforces (segment + market +
      // lower(btrim(name)) [+ address, which this app-level check does not
      // consider — see note below]).
      //
      // FIXED 2026-09-21: this used to be
      //   .or(`full_name.eq.${identifier},entity_name.eq.${identifier}`)
      // PostgREST's .or()/.and() read a bare comma as the clause separator,
      // and ACRIS names routinely contain one ("SMITH, JOHN") — 552 of 627
      // leads did, the day this was found. The filter silently parsed into
      // three malformed clauses, the lookup always returned nothing, and
      // every re-run of an ACRIS bridge re-inserted the same people. Once a
      // unique index existed to catch the resulting duplicates, the same
      // bug turned into a loud insert error instead — still wrong, just
      // louder. Fix: wrap each value in double quotes, which is PostgREST's
      // own escape for a comma (or any other special character) inside an
      // or()/and() value. ilike instead of eq also matches the index's
      // case-insensitive comparison — an exact eq() would miss two spellings
      // of the same name that the index (and a human) would consider the
      // same lead — and % / _ are escaped first since ilike treats them as
      // wildcards, not literal characters, and a name could contain either.
      //
      // Known remaining gap, not fixed here: this still matches on name
      // alone, not name+address like the unique index does. Two different
      // people who share a name in the same segment/market but live at
      // different addresses will collide here and one will get the other's
      // update instead of its own row — bounded by the per-lead try/catch
      // below (one skipped lead with a clear error, not data corruption),
      // but real. Fixing it means adding property_address to this match,
      // which needs its own normalization decision (exact address match is
      // too strict given free-text sources) and is being left as a
      // follow-up rather than folded into this fix.
      const namePattern = escapeOrValue(escapeIlikeWildcards(identifier));
      const { data: existing } = await supabase
        .from('isa_leads')
        .select('id, outreach_status')
        .eq('segment', segment)
        .eq('market', market)
        .or(`full_name.ilike.${namePattern},entity_name.ilike.${namePattern}`)
...
/** Escapes % and _ so an ilike comparison matches the value literally instead
 *  of treating them as SQL LIKE wildcards. */
function escapeIlikeWildcards(v: string): string {
  return v.replace(/[%_\\]/g, (c) => `\\${c}`);
}

/** PostgREST's or()/and() filter syntax reads a bare comma as the clause
 *  separator. Wrapping a value in double quotes is PostgREST's own escape for
 *  a comma (or any other special character) inside an or()/and() value;
 *  backslash-escape any literal double quote in the value first so the
